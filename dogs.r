...
dog_format_all = rbind(dog_format_all, data.frame(BreedName = rep(NA, times = 6), AnimalName = rep(NA, times = 6), rank = rep(21, times = 6), 
    year = c(2015, 2016, 2017, 2018, 2019, 2020), n_name = rep(NA, times = 6), n_rel_name = rep(NA, times = 6), n_breed = rep(NA, times = 6), n_rel_breed = rep(NA, times = 6), name_col = rep(NA, times = 6), breed_col = rep(NA, times = 6),
    prop_name = rep(0.1, times = 6), prop_breed = rep(-0.1, times = 6), name_lab = rep(NA, times = 6), breed_lab = rep(NA, times = 6)))

# plot with dark gray background
...
geom_tile(aes(y = prop_breed/2, group = BreedName, 
  ...
  theme(axis.line=element_blank(),
        ...
        plot.background=element_rect(fill = '#2D2D2D', color = '#2D2D2D'),
       plot.margin = margin(1,1,6,1, "cm")) + 
       geom_segment(aes(x = 0, xend = 21, y = 0, yend = 0), color = '#2D2D2D', size = 1)

# read image 
d = data.frame(x = 25, y = 0, image = "/Users/annebozack/Desktop/dogs_darkgray.png")
...
animate(anim, 200, fps = 20,  width = 1200, height = 1400, 
        renderer = gifski_renderer(file = '/Users/annebozack/Desktop/doganimation_darkgray.gif', loop = T))
